...
import type { ChannelBlockee } from '@/models/entities/ChannelBlockee.js';
import { QueueService } from '@/core/QueueService.js';
import { GlobalEventService } from '@/core/GlobalEventService.js';
import { DI } from '@/di-symbols.js';
import type { ChannelFollowRequestsRepository, ChannelBlockeeRepository, UserListsRepository, UserListJoiningsRepository } from '@/models/index.js';
...
import { ChannelFollowingService } from '@/core/ChannelFollowingsService.js';

@Injectable()
export class ChannelBlockeeService implements OnModuleInit {
	...
	constructor(
		...
		private followRequestsRepository: ChannelFollowRequestsRepository,

		@Inject(DI.channelBlockeeRepository)
		private channelBlockeeRepository: ChannelBlockeeRepository,
	...
	onModuleInit() {
		this.channelFollowingService = this.moduleRef.get('ChannelFollowingService');
	}

	@bindThis
	public async block(blocker: User, blockee: Channel, silent = false) {
		await Promise.all([
			this.cancelRequest(blockee, blocker, silent),
			this.channelFollowingService.unfollow(blocker, blockee, silent),
		...
		const blocking = {
			...
			channelId: blockee.id,
		} as ChannelBlockee;

		await this.channelBlockeeRepository.insert(blocking);
		...
		if ( this.userEntityService.isRemoteUser(blockee)) {
	...
	@bindThis
	private async cancelRequest(follower: Channel, followee: User, silent = false) {
		...
		if (this.channelEntityService.isLocalUser(follower) && this.userEntityService.isRemoteUser(followee)) {
